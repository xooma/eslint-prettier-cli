...
export const getMultipleConfigurations = async (configNames: Array<ProjectConfigType>): Promise<Array<EslintConfigStructure>> => Promise.all(configNames.map(getConfiguration));

export const mergeContentConfigurations = (configs: Array<EslintConfigStructure>): EslintConfigStructure => configs.reduce((acc, config) => mergeDeep(acc, config), {});

export const loadConfiguration = async (args: Array<ProjectConfigType>): Promise<EslintConfigStructure> => getMultipleConfigurations(args).then(mergeContentConfigurations);
